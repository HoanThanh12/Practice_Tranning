		...
		switch (srcName) {
        	...
        	case "btn_Down_Excel":
        		if(sheetObject1.RowCount() < 1){
        			...
        			sheetObject1.Down2Excel({ DownCols: makeHiddenSkipCol(sheetObject1),Merge:false,FileName:"Customer"});
        		} 	    	    	
    	        break;
			default :
				break;
...
function doActionIBSheet(sheetObj,formObj,sAction) {
	switch (sAction) {
		case IBSEARCH: // retrieve
			...
			ComOpenWait(true);
			sheetObj.DoSearch("CUSTOMER_TRN_004POPUPGS.do", FormQueryString(formObj));
			break;
	}
}
